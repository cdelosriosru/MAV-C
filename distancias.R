...
rios1<-readOGR(dsn="DATA/Rios_colombia1", layer="Rios_Nivel_2")
rios2<-readOGR(dsn="DATA/rios_colombia2", layer="Rios_Nivel_3")

rios<-raster::bind(rios1,rios2)
rios<-spTransform(rios, crs(consejos_coca)) 
writeOGR(rios, "." ,driver="ESRI Shapefile", layer="rios")

rios2 <- spTransform(rios, wgs.84)
...
for (i in 1:nrow(cents)){
  ...
  rio.proj <- spTransform(rios2,CRS(epsg.2062))
  MAD.proj   <- spTransform(MAD,CRS(epsg.2062))
  dis <- gDistance(MAD.proj,rio.proj) #EPSG-2062 projection, units are in meters.
  
  dis_rio <- rbind(dis_rio, dis)
}

cents$dis_rio <- dis_rio
cents2 = cents

consejos_coca_dta<- as.data.frame(coca2, xy=TRUE, na.rm=TRUE)

write.dta(consejos_coca_dta, "DATA/Procesada/consejos_coca.dta", convert.factors = "string") 
write.dta(cents2, "DATA/Procesada/dist_rios.dta", convert.factors = "string") 
...
# I also have to know wheter the points are inside the parks or outside them
# this is easy to do with an intersection
...
resguardos<-clgeo_Clean(resguardos)
dpto<-clgeo_Clean(dpto)
...
#-----distance to forest border---------

# this is a bit more difficult. I am going to use IDEAM data in 2000
